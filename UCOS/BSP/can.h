...
#define CAN0_MSG_NUMBER 10           /* 定义缓冲区存储单元大校最后一个缓冲区不存储数据 目前发现这个数字等于5时较为合适 */
#define CAN1_MSG_NUMBER 10           /* 定义缓冲区存储单元大校最后一个缓冲区不存储数据 目前发现这个数字等于5时较为合适 */
/*********************************************************************************************************
**   定义 CAN 报文收发项 结构体:
*********************************************************************************************************/
...
extern MessageDetail MsgCAN0BUF[CAN0_MSG_NUMBER];                        /*定义CAN缓冲曲  用于先进先出处理数据，防止丢包              */
extern MessageDetail MsgCAN1BUF[CAN1_MSG_NUMBER];                        /*定义CAN缓冲曲  用于先进先出处理数据，防止丢包              */
extern MessageDetail *pwMsgCAN0,*prMsgCAN0;
extern MessageDetail *pwMsgCAN1,*prMsgCAN1;
...
/*********************************************************************************************************
** Function name:  	unsigned char WriteCAN1(unsigned char LEN,unsigned char FF, unsigned int 
**                      ID, unsigned char *data);
** Descriptions: 	用户填写发送 帧信息
** input parameters:    LEN:         数据长度
**                      *data:       发送数据所在的数组 
** Returned value:      操作成功返回 1 失败返回 0
*********************************************************************************************************/
CANAPP_GLOBAL unsigned char WriteCAN1(unsigned char LEN,unsigned char FF,
                                        unsigned int ID,unsigned char *data);
