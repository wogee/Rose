...
MessageDetail MsgCAN0BUF[CAN0_MSG_NUMBER];                           /*定义CAN缓冲曲  用于先进先出处理数据，防止丢包              */
MessageDetail MsgCAN1BUF[CAN1_MSG_NUMBER];                           /*定义CAN缓冲曲  用于先进先出处理数据，防止丢包              */
MessageDetail *pwMsgCAN0 = MsgCAN0BUF;                              /*定义两个指针，写指针和读指针，指向缓冲区                   */
MessageDetail *prMsgCAN0 = MsgCAN0BUF;                              /*定义两个指针，写指针和读指针，指向缓冲区                   */
MessageDetail *pwMsgCAN1 = MsgCAN1BUF;                              /*定义两个指针，写指针和读指针，指向缓冲区                   */
MessageDetail *prMsgCAN1 = MsgCAN1BUF;                              /*定义两个指针，写指针和读指针，指向缓冲区                   */
    ...
    if(LEN>8){
        ...
        for(i=0;i<LEN;i++) {
            if(i<4){
				...
				CANSend(0, 1);
       OSTimeDlyHMSM(0, 0, 0, 1);                                   /*  防止发送过快 ,缓冲区未发送完毕又写入下一帧数据，不能发送               */
	 return(1);
    }
...
void CAN_IRQHandler (void)
{
    ...
    for(j=0;j<2;j++){				
       ...
       {					 
						CANRCV(j, pwMsgCAN0);    			 
	          if(pwMsgCAN0 == &MsgCAN0BUF[CAN0_MSG_NUMBER-1])                
					  { 
		         ...
		         /* 填写接收帧信息*/
//			  	 CANRCV(j, &MessageCAN0);                                       /* 收到CAN0中断,接收帧 放进缓冲区等待任务取出去处理               */  
...
//		       MessageDetailT.CANID=MessageCAN0.CANID;
      ...
      {
						CANRCV(j, pwMsgCAN1);    			 
	          if(pwMsgCAN1 == &MsgCAN1BUF[CAN1_MSG_NUMBER-1])                
					  { 
                 pwMsgCAN1 = MsgCAN1BUF;							                     /* 调整指针指向                                                   */
						}	
				   else                                                            /* 留出一个存储单元，防止读指针在指向最后一个存储单元又做加法     */
					  { 
                 pwMsgCAN1++;                                              /* 指针递增必须放在最后，防止指针递增后进行重复的处理             */
						}		
...
void CAN0_RECData_Pro(void)
{
	 ...
	 //移到  BMSMain任务下的   BMS_RECData_Pro  函数中
}
